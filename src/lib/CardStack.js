...
class CardStack extends React.Component {
  ...
  nextCard = (currentCardIndex) => {
    ...
    this.setState({ cards: newCards }, () =>
      _.delay(() => {
        this.setState({ cards: this.state.cards.filter(c => c !== fadingOutCard) })
    ...
    return (
      <CardsContainer>
        {this.state.cards.map((card, i) => {
          if (this.props.maxVisibleCards && (this.props.waitForFadeOut ? i : i - this.state.cards.filter(c => c.fadingOut).length) >= this.props.maxVisibleCards)
            return undefined;
